...
static int _have_ipv4() 
{
    ...
    return _test_connect(AF_INET, (struct sockaddr *)&sin_test, sizeof(sin_test));
}
bool checknetfamily()
{
...
int socketv6_gethostbyname(const char* _host, socket_ipinfo_t* _ipinfo, int _timeout /*ms*/, const char* _dnsserver) {
    ...
    if(!_dnsserver) {
	_dnsserver = "2001:67c:27e4:15::64";
    }
...
int socketv4_gethostbyname(const char* _host, socket_ipinfo_t* _ipinfo, int _timeout /*ms*/, const char* _dnsserver) {
    ...
    dest.sin_family = AF_INET;
            printf("dnsserver:%s\n",dns_servers[0].c_str());
    inet_pton(AF_INET, dns_servers[0].c_str(), &(dest.sin_addr)); 
    ...
    if (fin.good()) {
        while (!fin.eof()) {
            if (fin.getline(str, LINE_LENGTH).good()) {
                std::string s(str);
                int num = s.find(NAME_SVR);
                //int num = s.find("nameserver");

                if (num >= 0) {
                    std::remove_if(s.begin(), s.end(), isspace);
                    s.erase(0, NAME_SVR_LEN);
